...
export class PdfService {
    ...
    async generateServicioPdf(idServicio: number): Promise<Buffer> {
        ...
        const servicio = await this.serviciosRepository.findOne({
            where: { idServicio },
            relations: ['cliente', 'sucursal', 'equipo', 'tecnico', 'tipoServicio', 'fotos'],
        });
        ...
        return new Promise((resolve, reject) => {
            ...
            doc.text(`Fecha: ${new Date(servicio.fechaServicio).toLocaleDateString()}`);
            doc.moveDown();
    ...
    async generateReportePorFecha(fechaInicio: Date, fechaFin: Date): Promise<Buffer> {
        ...
        return new Promise((resolve, reject) => {
            ...
            servicios.forEach((servicio) => {
                ...
                doc.text(new Date(servicio.fechaServicio).toLocaleDateString(), 110, y, { width: 70 });
            ...
            // Summary
            doc.moveDown(2);
            doc.fontSize(10).text(`Total de servicios: ${servicios.length}`, { align: 'right' });
